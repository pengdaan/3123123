...
from app.libs.httprunner import (
    __version__,
    exceptions,
    loader,
    logger,
...
class HttpRunner(object):
    ...
    def _add_tests(self, testcases, case_id=None):
        ...
        def _add_test(test_runner, test_dict, case_id=None):
            """add test to testcase."""

            def test(self):
                try:
                    test_runner.run_test(test_dict, case_id)
                except exceptions.MyBaseFailure as ex:
                    self.fail(str(ex))
                finally:
                    # print('test_runner.meta_datas',test_runner.meta_datas)
                    self.meta_datas = test_runner.meta_datas
                    self.meta_datas["end_hook"] = test_runner.now_run_hook
        ...
        for testcase in testcases:
            ...
            for index, test_dict in enumerate(tests):
                ...
                for times_index in range(times):
                    ...
                    test_method = _add_test(test_runner, test_dict, case_id)
    ...
    def run_tests(self, tests_mapping, case_id=None):
        ...
        test_suite = self._add_tests(parsed_testcases, case_id)
        # run test suite
    ...
    def run(self, path_or_tests, dot_env_path=None, mapping=None, case_id=None):
        """main interface.
        ...
        if loader.is_test_path(path_or_tests):
            ...
            # print("这里开始第一步:")
            return self.run_tests(path_or_tests, case_id)
        else:
            raise exceptions.ParamsError(
