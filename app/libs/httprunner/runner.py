...
from app.libs.httprunner import exceptions, logger, response, utils
from app.libs.httprunner.client import HttpSession
from app.libs.httprunner.context import SessionContext
from app.libs.httprunner.validator import Validator
...
class Runner(object):
    ...
    def do_hook_actions(self, actions, hook_type):
        ...
        for action in actions:
            if isinstance(action, dict) and len(action) == 1:
                ...
                self.now_run_hook.append(action)
                self.session_context.eval_content(action)

    def _run_test(self, test_dict, case_id=None):
        """run single teststep.
        ...
        try:
            ...
            self.session_context.update_session_variables(
                extracted_variables_mapping, case_id
            )
    ...
    def run_test(self, test_dict, case_id=None):
        ...
        if "teststeps" in test_dict:
            ...
            try:
                self._run_test(test_dict, case_id)
            except Exception:
                # log exception request_type and name for locust stat
