...
def parse_parameters(parameters, variables_mapping=None, functions_mapping=None):
    # print('parse_parameters:',parameters)
...
def _parse_testcase(testcase, project_mapping, session_variables_set=None):
    ...
    try:
        ...
        for i in testcase["teststeps"]:
            if "name" in i and "skipUnless" in i:
                error_case_name = i["name"] + ":"
        testcase_type = testcase["type"]
        testcase_path = testcase.get("path")
        ERROR["msg"] = "failed to parse testcase, error: {}{}".format(
            error_case_name, ex
        )
        logger.log_error(
        ...
        raise ValueError(
            "failed to parse testcase, error: {}{}".format(error_case_name, ex)
        )
...
def __get_parsed_testsuite_testcases(testcases, testsuite_config, project_mapping):
    ...
    for testcase_name, testcase in testcases.items():
        ...
        if "parameters" in testcase and testcase["parameters"]:
            ...
            for parameter_variables in cartesian_product_parameters:
                # print('cartesian_product_parameters',cartesian_product_parameters)
    ...
    # print('parsed_testcase_list',parsed_testcase_list)
    return parsed_testcase_list


def _parse_testsuite(testsuite, project_mapping):
    # print('_parse_testsuite',testsuite)
        ...
        if test_type == "testsuites":
            ...
            for testsuite in testsuites:
                ...
                for parsed_testcase in parsed_testcases:
                    # print('eeqweqeqweqwe', parsed_testcases)
...
def _parameters(tests_mapping):
    ...
    if "parameters" in tests_mapping["testcases"][0]["config"]:
        ...
        for testcase in tests_mapping["testcases"][0]["teststeps"]:
            ...
            new_teststeps += new_case_list
        tests_mapping["testcases"][0]["teststeps"] = new_teststeps
        # print('new_teststeps',new_teststeps)


def get_testcase(testcase, base_tests_mapping):
    # 判断api_data是否含有json
    try:
        keylist = []
        case_json = testcase["request"]["json"]
        if isinstance(case_json, dict):
            for i in case_json.keys():
                if isinstance(case_json[i], str):
                    if len(re.findall("p{(.*?)}", case_json[i])) > 0:
                        keylist.append(i)
                    else:
                        continue
                else:
                    break
        if len(keylist) > 0:
            new_testData = update_testData(
                testcase, case_json, keylist, base_tests_mapping
            )
            return new_testData
        else:
            return [testcase]
...
def update_testData(testcase, case_json, keylist, base_tests_mapping):
    """[summary]
    封装parameters到json中

    Args:
        testcase ([type]): [description]
        case_json ([type]): [description]
        keylist ([type]): [description]
        base_tests_mapping ([type]): [description]

    Returns:
        [type]: [description]
    """
    index = []
    for i, dic in enumerate(base_tests_mapping):
        for key in keylist:
            if key in keylist:
                index.append(dic)
    for k in keylist:
        if k in case_json.keys():
            case_json.pop(k)
    for new_key in case_json:
        new_json = {new_key: [case_json[new_key]]}
        index.append(new_json)
    parameters = parse_parameters(index)
    # print('parse_parameters====1111=》', parameters)
    testcaselist = []
    count = 1
    if len(parameters):
        for p_case in parameters:
            # 深度拷贝
            my_testcase = copy.deepcopy(testcase)
            my_testcase["name"] = "Test_" + str(count)
            my_testcase["request"]["json"] = p_case
            testcaselist.append(my_testcase)
            count = int(count) + 1
    else:
        my_testcase = copy.deepcopy(testcase)
        testcaselist.append(my_testcase)
    # print('testcaselist:', testcaselist)
    return testcaselist
