                ...
                if _method == "POST":
                    if "body" in v:
                        if isinstance(v["body"], list):
                            api_module_detail = _erase._api_json(k, k, _method, _headers, _dheaders, _params, _dparams, v["body"])
                        else:
                            api_module_detail = _erase._form_data(k, k, _method, _headers, _dheaders, _params, _dparams, v["body"])
                    else:
                        api_module_detail = _erase._api_json(k, k, _method, _headers, _dheaders, _params, _dparams, "")
                else:
                    api_module_detail = _erase._api_get_json(k, k, _method, _headers, _dheaders, _params, _dparams)
                Module.add_api_module("capture_" + str(now_time), str(api_module_detail), api_id, res['user_id'])
            else:
                Fail(msg=str(k) + "解析失败")
    return Sucess()
